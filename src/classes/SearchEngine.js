...
export class SearchEngine {
    ...
    searchRecipes(type, inputValue, array) {
        let targetProps = [];
        type === "main" ? (targetProps = ["name", "ingredients", "description"]) : (targetProps = [type]);
        ...
        const results = array.filter((data) => {
            for (let prop of targetProps) {
                ...
                if (prop === "ingredients") {
                    ...
                    data[prop].toLowerCase().indexOf(input) !== -1 ? (validRecipe = true) : (validRecipe = false);
                }
                if (validRecipe) {
                    return true;
                }
    ...
    multiSearch(array, searches, newPass) {
        ...
        if (newPass == true) {
            for (let search in searches) {
                if (search == 0) {
                    multiResults = this.searchRecipes(searches[search][0], searches[search][1], array);
                } else {
                    multiResults = this.searchRecipes(searches[search][0], searches[search][1], multiResults);
                }
            }
        } else {
            multiResults = this.searchRecipes(searches[searches.length - 1][0], searches[searches.length - 1][1], array);
        }
        return multiResults;
    }
    watchSearches() {
    ...
    mainSearch(e) {
        if (e.target.value.length >= 3 && e.data !== " ") {
            ...
            let results = [];
            if (e.data !== null) {
                results = this.multiSearch(this.filteredRecipes, this.searches, this.newPass);
            } else {
                results = this.multiSearch(this.allRecipes, this.searches, this.newPass);
            }
            this.newPass = false;
            if (!results.length) {
    ...
    removeTag(search) {
        ...
        if (this.searches.length > 0) {
            this.filteredRecipes = this.allRecipes;
            let results = this.multiSearch(this.filteredRecipes, this.searches, this.newPass);
            this.newPass = false;
            if (!results.length) {
                Cards.createAllCards([]);
                this.dropdowns.generateOptions([]);
